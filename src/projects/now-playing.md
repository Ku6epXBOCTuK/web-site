---
title: now_playing
type: widget
subtitle: obs browser source
description:
  obs browser source widget that displays the currently playing track. reads
  from a simple text file updated by your music player.
tags:
  - html
  - css
  - javascript
image: images/now-playing.jpg
url: https://github.com/Ku6epXBOCTuK/now_playing
---

## now_playing

obs browser source widget that displays the currently playing track.

## Usage

1. add as browser source in obs
2. point to music player's now playing file
3. done - zero dependencies
